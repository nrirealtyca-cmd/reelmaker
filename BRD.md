...
- AI-generated captions overlaid on video
...
| ID | Requirement | Priority | Status |
|----|-------------|----------|--------|
| FR-05.1 | Generate captions from photo metadata or AI analysis | Must | ❌ Not Started |
| FR-05.2 | Overlay captions on video using FFmpeg drawtext | Must | ❌ Not Started |
| FR-05.3 | Caption styling consistent with design system | Should | ❌ Not Started |
| FR-05.4 | Option to generate without captions | Should | ❌ Not Started |
...
| FR-06.4 | Deep link to Instagram for sharing | Could | ❌ Not Started |
| FR-06.5 | Deep link to YouTube for sharing | Could | ❌ Not Started |
...
**Flow**: User clicks "Generate Videos" → processing screen with progress bar → backend downloads media → FFmpeg encodes Reel → stream-copies Short → validates both → preview screen

### UC-04: AI Caption Generation ❌
**Actor**: User
**Flow**: During generation, AI analyzes selected media → generates contextual captions → FFmpeg burns captions into video → user previews captioned video

### UC-05: Download and Share ⚠️
**Actor**: User
**Flow**: On preview screen → user clicks "Download" to save .mp4 → or clicks "Share" to use Web Share API (mobile) → future: deep links to Instagram/YouTube apps
...
| 2.0 | March 26, 2026 | Session 4 | Updated status for all FRs, added NFRs, added FR-03.8/FR-03.9 for queue and memory optimization, updated UC-03 flow |
